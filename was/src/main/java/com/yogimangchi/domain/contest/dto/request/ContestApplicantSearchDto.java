...
@Schema(description = "대회 신청자 목록 검색 조건 / 무한 스크롤")
public record ContestApplicantSearchDto(
        @Schema(description = "첫 요청은 비워두고, 다음 요청부터는 이전 응답의 nextCursorId 값을 넣어주세요.", example = "120", nullable = true)
        Long cursorId,

        @Schema(description = "신청 상태 필터입니다. 비우면 전체 상태를 조회합니다.", example = "PENDING", nullable = true)
